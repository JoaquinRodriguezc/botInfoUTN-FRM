...
import { convertMsgToQuery } from "./openia.js";
import debounce from "./utils.js";
const PORT = process.env.PORT ?? 3008;
const USE_OPEN_IA = process.env.USE_OPEN_IA === "true";
const mainFlow = addKeyword("").addAction(async (ctx, { state, gotoFlow, endFlow, flowDynamic }) => {
    if (ctx.body.includes("event_media") ||
        ...
        return endFlow("Soy buen lector pero no puedo leer imagenes  o escuchar tu audio :(");
    }
 ...
 if (ctx.body.toLowerCase().match("gracias")) {
    ...
    const debunced = debounce(async () => {
        if (USE_OPEN_IA) {
            ...
            try {
                const userQuery = await convertMsgToQuery(ctx.body);
                console.log("Converted query", userQuery);
                if (userQuery.error) {
                    state.update({ from: null });
                    return endFlow(userQuery.error);
                }
...
const horariosFlow = addKeyword(EVENTS.ACTION).addAction(async (ctx, { state, flowDynamic, endFlow }) => {
    const { materia, comision } = state.get("data");
    if (!materia || !comision) {
        state.update({ from: null });
        console.log("Usuario agregado borrado de estado respuesta en curso");
        return endFlow("No se encontró información para la materia y comisión mandada");
    }
...
const apuntesFlow = addKeyword("/apuntes").addAnswer("Página web con apuntes de alumnos: https://apuntesutnmza.com");
const mesasFlow = addKeyword(EVENTS.ACTION).addAnswer("Buscando mesa...", null, async (ctx, { state, flowDynamic }) => {
    if (!USE_OPEN_IA) {
